...
async function computePopup() {
  ...
  // Atribución por CLIC: para cada comprador, el último clic en un correo ANTES
  // de cada pedido atribuye la venta de ese pedido a ese correo/campaña. Así se
  // ve cuánta plata compra la base del pop-up a través de cada mail.
  const attrib = await mapPool(buyerList, async (b) => {
    let clicks = [];
    try {
      const a = await mcGet(`/lists/${LID}/members/${md5(b.email)}/activity?count=200&action=click`);
      clicks = (a.activity || [])
        .filter((x) => x.action === "click")
        .map((x) => ({ ts: d(x.timestamp), title: x.title || "(correo sin título)" }))
        .sort((x, y) => x.ts - y.ts);
    } catch (_) {}
    const perOrder = b.orders.map((od) => {
      let last = null;
      for (const c of clicks) { if (c.ts < od.date) last = c; else break; }
      return { total: od.total, date: od.date, title: last ? last.title : null };
    });
    return { email: b.email, isNew: b.isNew, perOrder };
  }, 9);

  const attrByEmail = {};
  // Compras de clientes pop-up por correo, SEPARADAS POR MES DE LA COMPRA, para
  // poder mostrar solo los correos del mes en curso (no todo el histórico).
  const byCampMonth = {}; // "YYYY-MM" -> { título -> {orders, venta} }
  for (const r of attrib) {
    let sales = 0, n = 0;
    for (const po of r.perOrder) {
      if (!po.title) continue;
      sales += po.total; n += 1;
      const m = monthKey(po.date);
      byCampMonth[m] = byCampMonth[m] || {};
      byCampMonth[m][po.title] = byCampMonth[m][po.title] || { title: po.title, orders: 0, venta: 0 };
      byCampMonth[m][po.title].orders += 1;
      byCampMonth[m][po.title].venta += po.total;
    }
    // La "venta atribuida" del KPI sigue siendo solo cohorte Nuevo.
    attrByEmail[r.email] = { attributedSales: r.isNew ? sales : 0, attributedOrders: r.isNew ? n : 0 };
  }
  const popupByCampaignMonth = {};
  for (const m of Object.keys(byCampMonth)) {
    popupByCampaignMonth[m] = Object.values(byCampMonth[m])
      .map((x) => ({ ...x, venta: Math.round(x.venta) }))
      .sort((a, b) => b.venta - a.venta);
  }

  // Compras de la base del pop-up por MES (todos los pedidos, atribuidos o no).
  const spendByMonth = {};
  for (const b of buyerList) for (const od of b.orders) {
    const m = monthKey(od.date);
    spendByMonth[m] = spendByMonth[m] || { mes: m, orders: 0, venta: 0 };
    spendByMonth[m].orders += 1;
    spendByMonth[m].venta += od.total;
  }
  const popupByMonth = Object.values(spendByMonth)
    .map((x) => ({ ...x, venta: Math.round(x.venta) }))
  ...
  return {
    ...
    popupByMonth,          // compras de clientes pop-up por mes (Shopify)
    popupByCampaignMonth,  // compras pop-up por correo, agrupadas por mes de la compra
    notas: {
...
export const getPopupTable = unstable_cache(computePopup, ["cava-popup-v4"], { revalidate: CACHE_S });
