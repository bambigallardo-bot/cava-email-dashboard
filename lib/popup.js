...
async function computePopup() {
  ...
  const attrByEmail = {};
  const byCamp = {}; // título de correo -> compras de clientes pop-up
  for (const r of attrib) {
    let sales = 0, n = 0;
    for (const po of r.perOrder) {
      if (!po.title) continue;
      sales += po.total; n += 1;
      byCamp[po.title] = byCamp[po.title] || { title: po.title, orders: 0, venta: 0 };
      byCamp[po.title].orders += 1;
      byCamp[po.title].venta += po.total;
    }
  ...
  const popupByCampaign = Object.values(byCamp)
  ...
  return {
    ...
    popupByCampaign,  // compras de clientes pop-up atribuidas a cada correo
    notas: {
...
export const getPopupTable = unstable_cache(computePopup, ["cava-popup-v3"], { revalidate: CACHE_S });
