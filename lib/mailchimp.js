...
// Meta Ads: campaña de registros ACTIVA (adquisición del pop-up). El dashboard
// no llega al MCP de Meta, así que se siembra por env (se actualiza a mano).
const META_ADS = {
  campaign: process.env.META_CAMPAIGN_NAME || "CAVA - 45% OFF Primera Compra - Septiembre 2026",
  status: process.env.META_STATUS || "ACTIVA",
  spend: Number(process.env.META_SPEND || 97589),
  results: Number(process.env.META_RESULTS || 1537),
  resultLabel: process.env.META_RESULT_LABEL || "Visitas a la landing",
  cpr: Number(process.env.META_CPR || 63),
  reach: Number(process.env.META_REACH || 50407),
  impressions: Number(process.env.META_IMPRESSIONS || 95310),
  updated: process.env.META_UPDATED || "2026-09-25",
};
...
export const getDashboard = unstable_cache(
  computeDashboard,
  ["cava-email-dashboard-v6"], // subir versión invalida la caché tras cambios de lógica
