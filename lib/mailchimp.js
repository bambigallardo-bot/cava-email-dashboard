...
// La tienda es CLP, pero Mailchimp reporta el revenue dividido por 100 (trata
// toda moneda como si tuviera 2 decimales; el CLP no los tiene). CLP real = valor * 100.
// Calibrable contra Shopify si hiciera falta.
const REVENUE_CLP_FACTOR = Number(process.env.REVENUE_CLP_FACTOR || 100);
...
function classifyCampaign(meta, shopifyIds, nameById) {
  ...
  // Respaldo: leer el texto del segmento (por si viene sin condiciones estructuradas).
...
function buildCampaign(rep, meta, cls) {
  ...
  return {
    ...
    revenue: Math.round((eco.total_revenue || 0) * REVENUE_CLP_FACTOR), // CLP
    currency: "CLP",
    world: cls.world,
    segmentNames,
...
export const getDashboard = unstable_cache(
  computeDashboard,
  ["cava-email-dashboard-v2"], // subir versión invalida la caché tras cambios de lógica
